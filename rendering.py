...
class Camera:
    ...
    def __init__(self, pos: Vec3, dir: Vec3, near_plane: Vec3, viewport_size: Vec3) -> None:
        self.pos = pos
        self.dir = dir
    ...
    def gen_primary_ray(self, screen_x: float, screen_y: float, screen_width: float, screen_height: float):
        ...
        ray_dir = bottom_left.add(adjacent.scale(self.viewport_size.x) * ((screen_x + rand_float(-0.5, 0.5)) / screen_width)).add(local_up.scale((self.viewport_size.y) * ((screen_y + rand_float(-0.5, 0.5)) / screen_height))).add(self.dir.scale(self.near_plane)).normalize()
